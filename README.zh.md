...
| 指标 | 优化前 | 优化后 |
| --- | --- | --- |
| 搜索端到端 | 118.5 秒 | 45.6 秒（-61.5%） |
| 首条结果 | 47.5 秒 | 12.8 秒（-73.1%） |
| 每轮 LLM 调用 | 41 次 | 20 次（-51.2%） |

各版本详见[版本亮点](docs/release-highlights.zh.md)，每个数字的测量方式见
[CHANGELOG.md](CHANGELOG.md) 的 `### Validation` 条目。
